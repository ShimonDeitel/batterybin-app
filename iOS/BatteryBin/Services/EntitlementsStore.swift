...
/// StoreKit 2 — auto-renewable subscription (`com.shimondeitel.batterybin.pro.monthly`).
...
final class EntitlementsStore: ObservableObject {
    static let productID = "com.shimondeitel.batterybin.pro.monthly"
    ...
    private var debugForcePro: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["BATTERYBIN_FORCE_PRO"] == "1"
        #else
        return false
        #endif
    }
    ...
    init() {
        ...
        Task {
            #if DEBUG
            if ProcessInfo.processInfo.environment["BATTERYBIN_NO_SK"] != "1" { await loadProduct() }
            #else
            await loadProduct()
            #endif
